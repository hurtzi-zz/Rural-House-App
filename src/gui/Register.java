...
public class Register extends JFrame {
	...
	public Register() {
		...
		lblSignIn.setBounds(167, 0, 106, 25);
		lblSignIn.setFont(new Font("Tunga", Font.BOLD, 22));
		contentPane.add(lblSignIn);
		
		JLabel lblUser = new JLabel("User");
		lblUser.setBounds(35, 21, 46, 14);
		contentPane.add(lblUser);
		
		JLabel lblOwner = new JLabel("Owner");
		lblOwner.setBounds(313, 21, 46, 14);
		...
		CCuadrado.setBounds(10, 178, 174, 40);
		...
		btnUser.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(!name.equals("")&& !surname.equals("")&& !user.equals("")&& !pass.equals("")){
					try{
						ApplicationFacadeInterface facades = StartWindow.getBusinessLogic();
						Client j=facades.verifyLogin(user,pass);
						if(j==null){
							facades.createClient(name, surname, user, pass, true);
							...
							CCuadrado.setText("Erabiltzailea sortuta zegoen.");
					...
					JOptionPane.showMessageDialog(null, "Bete itzazu atal guztiak mesedez", "alert", JOptionPane.CANCEL_OPTION);
				}
			}
		});
		...
		separator.setBounds(193, 36, 15, 214);
